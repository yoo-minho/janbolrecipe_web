...
<div class="container">
    ...
    ?>
      ...
      <form name ="check_form" action="edit_process.php?class=<?=$class?>&recipe=<?=$recipe?>"
        method="post" onsubmit="return checkform()">
          <input class='btn btn-warning' type="submit" value="수정 완료">
          ...
          <div class="row">
            ...
            <div class="form-group col-sm-4">
              <label for="recipe">youtube.com/watch?v=</label>
                ...
                type="text" name="video_code" value=<?=$row['video_code']?>>
            </div>
          </div>
                 ...
                 $seasoning_array= array('간장', '설탕', '액젓', '고추장', '식용유', '참기름', '고춧가루', '다진마늘');
                   ...
                   <div class="col-sm-3">
                     ...
                     <select class="form-control" id="sel1" name="seasoning<?=$i?>">
                       <?php
                       for ($j=0; $j <11 ; $j++) {
                         if($j == (int)substr($row['seasoning'],$i+1,1)){
                 ...
                 <div class="col-sm-4">
                   <div class="panel" style="padding:10px;">
                     <div class="panel-body" >
                       ...
                       type="number" name="gram<?=$i?>" min=0
                       value=<?=(int)$row['gram'.$i]?>>
                     </div>
                   </div>
                 </div>
               <?php } ?>
